  ...
  const connectToDevice = async (device, retryCount = 0) => {
    ...
    try {
      ...
      if (connectionCancelled.current) {
        console.log("⚠️ Cancelled before connection");
        return;
      }
      ...
      if (connectionCancelled.current) {
        console.log("⚠️ Cancelled after connection, cleaning up...");
        await manager.cancelDeviceConnection(device.id);
        return;
      }
      ...
      if (connectionCancelled.current) {
        console.log("⚠️ Cancelled after discovery, cleaning up...");
        await manager.cancelDeviceConnection(device.id);
        return;
      }
      ...
      console.log("✅ Fully connected:", updatedName);
      alert(`✓ Connected to ${updatedName}`);
